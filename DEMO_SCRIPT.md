...
Chrome profile, have one short video clip (~15–20s) and one song file on the
desktop. Press `M` once in the editor if you want the fast export path.
...
| 15–25s | **Drag both files** from the desktop straight into the window. Point at the waveform appearing. | "Drop in a song and a clip. It decodes the audio, draws the waveform, and detects the BPM — all client-side, off the main thread." |
...
| 45–60s | Click **Export** → pick **TikTok** → **Export**. Let the progress bar run while you talk. | "And it encodes real H.264 in the browser — FFmpeg WASM, fully offline. On stage wifi this still works, because there's nothing to download." |
